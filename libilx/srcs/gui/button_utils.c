...
/*   Updated: 2023/02/15 16:42:55 by lcrimet          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "gui.h"

static void	ft_cp_button(t_button *src, t_button *dest)
{
	dest->rect = src->rect;
...
int	ilx_add_button(t_gui *gui, t_button *add_button, void *(*func)(void *))
{
	...
	ft_cp_button(add_button, &new_buttons[i]);
	free(add_button);
	new_buttons[i].f = func;
	new_buttons[i + 1].label = NULL;
	free(gui->buttons);
	gui->buttons = new_buttons;
	return (1);
}
